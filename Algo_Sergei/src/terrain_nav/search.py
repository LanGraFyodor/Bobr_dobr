...
def localize_from_nmea(
    ...
    # Apply median filter to remove altimeter spikes
    smoothed_terrain_profile = medfilt(profile.terrain_profile_m, kernel_size=5)
    ...
    with rasterio.open(dem_path) as dataset:
        ...
        terrain_variance = float(np.nanvar(smoothed_terrain_profile))
        if terrain_variance <= flat_variance_threshold_m2:
            ...
            return _flat_result(smoothed_terrain_profile, profile.timestamps_s, grid, old_x, old_y, terrain_variance)
        ...
        errors, best_flat_index = compute_error_grid(
            ...
            utm_crs=utm_crs,
            measured_profile_m=smoothed_terrain_profile,
...
def localize_position_from_nmea(
    ...
    with rasterio.open(dem_path) as dataset:
        ...
        if terrain_variance <= flat_variance_threshold_m2:
            ...
            return _flat_result(smoothed_terrain_profile, profile.timestamps_s, coarse_grid, old_x, old_y, terrain_variance)
